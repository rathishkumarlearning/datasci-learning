...
import {
  ...
  Lightbulb, AlertTriangle, Play, Home, BarChart3
} from 'lucide-react';
import './index.css';
...
function App() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('datasci-learning-progress');
  ...
  const saveProgress = (lessonId) => {
    ...
    localStorage.setItem('datasci-learning-progress', JSON.stringify(updated));
  };
                ...
                {rows.map((row, ri) => (
                  <tr key={ri}>{row.map((cell, ci) => (
                    <td key={ci} style={{ padding: '12px 16px', borderBottom: '1px solid var(--border-color)', color: 'var(--text-secondary)' }}>
                      <span dangerouslySetInnerHTML={{ __html: cell.replace(/\*\*(.*?)\*\*/g, '<strong style="color:var(--text-primary)">$1</strong>').replace(/`(.*?)`/g, '<code style="background:rgba(20,184,166,0.1);padding:2px 6px;border-radius:4px;font-size:13px">$1</code>') }} />
                    </td>
                  ))}</tr>
                ))}
              </tbody>
      ...
      if (line.startsWith('- ') || line.startsWith('* ')) {
        ...
        elements.push(
          <ul key={`ul-${i}`}>{items.map((item, idx) => (
            <li key={idx} dangerouslySetInnerHTML={{ __html: item.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/`(.*?)`/g, '<code style="background:rgba(20,184,166,0.1);padding:2px 6px;border-radius:4px">$1</code>') }} />
          ))}</ul>
        );
        continue;
      }
      ...
      if (line.trim()) {
        elements.push(
          <p key={`p-${i}`} dangerouslySetInnerHTML={{ __html: line
            ...
            .replace(/`(.*?)`/g, '<code style="background:rgba(20,184,166,0.1);padding:2px 6px;border-radius:4px;font-family:JetBrains Mono,monospace;font-size:14px">$1</code>')
  ...
  return (
    <div className="app-container">
      ...
      <div className="gradient-orb orb-secondary" />
      ...
      <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`}>
        <div className="sidebar-header">
          <div className="sidebar-logo"><BarChart3 size={24} /></div>
          <span className="sidebar-title">Data Science</span>
        </div>
      ...
      <main className="main-content">
        {currentView === 'home' && (
          <>
            <div className="hero">
              <div className="hero-badge"><BarChart3 size={16} /><span>Data Science Course</span></div>
              <h1 className="hero-title">Master<br /><span>Data Science</span></h1>
              <p className="hero-subtitle">Learn Pandas, NumPy, data visualization, and statistical analysis. Transform raw data into insights!</p>
            ...
            <div className="glass-card" style={{ textAlign: 'center' }}>
              ...
              <p style={{ color: 'var(--text-secondary)', marginBottom: '24px' }}>By the end, you'll be able to analyze any dataset:</p>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '16px' }}>
                {['NumPy Arrays', 'Pandas DataFrames', 'Data Cleaning', 'Visualization', 'Statistics', 'ML Basics'].map((item) => (
                  <div key={item} style={{ padding: '16px', background: 'rgba(20, 184, 166, 0.1)', borderRadius: '12px', border: '1px solid rgba(20, 184, 166, 0.2)' }}>
